...
public class ProviderApplicationInitListener extends ApplicationInitListener {
	...
	protected void customInit(final ContextRefreshedEvent event) {
		...
		final ServiceRegistryRequestDTO createCarServiceRequest = createServiceRegistryRequest(SkeletonProviderConstants.CREATE_CAR_SERVICE_DEFINITION, SkeletonProviderConstants.CAR_URI, HttpMethod.POST);
		arrowheadService.forceRegisterServiceToServiceRegistry(createCarServiceRequest);

		ServiceRegistryRequestDTO getCarServiceRequest = createServiceRegistryRequest(SkeletonProviderConstants.GET_CAR_SERVICE_DEFINITION,  SkeletonProviderConstants.CAR_URI, HttpMethod.GET);
		getCarServiceRequest.getMetadata().put(SkeletonProviderConstants.REQUEST_PARAM_KEY_BRAND, SkeletonProviderConstants.REQUEST_PARAM_BRAND);
		getCarServiceRequest.getMetadata().put(SkeletonProviderConstants.REQUEST_PARAM_KEY_COLOR, SkeletonProviderConstants.REQUEST_PARAM_COLOR);
	...
	private ServiceRegistryRequestDTO createServiceRegistryRequest(final String serviceDefinition, final String serviceUri, final HttpMethod httpMethod) {
		...
		if (sslEnabled && tokenSecurityFilterEnabled) {
			...
			serviceRegistryRequest.setInterfaces(List.of(SkeletonProviderConstants.INTERFACE_SECURE));
		} else if (sslEnabled) {
			systemRequest.setAuthenticationInfo(Base64.getEncoder().encodeToString(arrowheadService.getMyPublicKey().getEncoded()));
			...
			serviceRegistryRequest.setInterfaces(List.of(SkeletonProviderConstants.INTERFACE_INSECURE));
		}
		serviceRegistryRequest.setProviderSystem(systemRequest);
		serviceRegistryRequest.setServiceUri(serviceUri);
		serviceRegistryRequest.setMetadata(new HashMap<>());
		serviceRegistryRequest.getMetadata().put(SkeletonProviderConstants.HTTP_METHOD, httpMethod.name());
