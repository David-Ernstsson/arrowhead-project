...
@Component
public class ElectricityPriceMonitorApplicationInitListener extends ApplicationInitListener {
	...
	private final Logger logger = LogManager.getLogger(ElectricityPriceMonitorApplicationInitListener.class);
	...
	private ServiceRegistryRequestDTO createDummyServiceRegistryRequest() {
		...
		serviceRegistryRequest.setServiceDefinition("electricity-price-monitor-dummy");
		...
		if (sslEnabled && tokenSecurityFilterEnabled) {
			...
			serviceRegistryRequest.setInterfaces(List.of(ElectricityPriceMonitorConstants.INTERFACE_SECURE));
		} else if (sslEnabled) {
			systemRequest.setAuthenticationInfo(Base64.getEncoder().encodeToString(arrowheadService.getMyPublicKey().getEncoded()));
			...
			serviceRegistryRequest.setInterfaces(List.of(ElectricityPriceMonitorConstants.INTERFACE_INSECURE));
		}
